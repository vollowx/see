...
section). The `<component>` does not necessarily match the name of the
component, for example `<md-loading>` is imported from
`@vollowx/seele/m3/loading-indicator/loading-indicator.js`. Such relationship
is documented on the page of each component.
...
- [Material You Expressive](./05-Material_You_Expressive/00-Overview.md), coded as `m3`
- [Windows 98](./06-Windows_98/00-Overview.md), coded as `win98`

Each design system contains not only conponents, but also assets like styles
and helper utils. To check them out, see `00_Overview` for each design system.
